...
def test_one_image(net, img):
    ...
    # crossline
    up = np.concatenate((final_mask[0][:, :135, :], final_mask[2][:, -135:, :]), axis=1)
    down = np.concatenate((final_mask[1][:, :135, :], final_mask[3][:, -135:, :]), axis=1)
    final = np.concatenate((up[:20, :, :], down[-20:, :, :]), axis=0)
    final_data = final[0, :, :]
    for j in range(D - 1):
